...
class Test_Pathway(TestCase):
    ...
    def setUp(self):
        ...
        self.products = {self.target_id: 1, "MNXM1": 2}
        self.rxn = Reaction(
            id="rxn_4",
            ec_numbers=["1.13.11.1"],
        ...
        self.id = "pathway"
        self.pathway = Pathway(id=self.id)
    ...
    def test_replace_reaction(self):
        ...
        self.assertEqual(self.pathway.get_reaction(self.rxn.get_id()), _rxn)
        self.assertTrue(self.pathway.replace_reaction(self.rxn.get_id(), rxn))
        self.assertNotEqual(self.pathway.get_reaction(self.rxn.get_id()), _rxn)

    def test_replace_reaction_wrong_rxnid(self):
        self.assertFalse(
            self.pathway.replace_reaction("wrong_rxn_id", Reaction(id="test"))
        )

    def test_get_id(self):
        self.assertEqual(self.pathway.get_id(), self.id)

    def test_get_nb_reactions(self):
        self.assertEqual(self.pathway.get_nb_reactions(), len(self.reactions))

    def test_get_nb_species(self):
        # build the list of all species from all reactions
        species = [
            rxn.get_products_compounds() + rxn.get_reactants_compounds()
            for rxn in self.pathway.get_list_of_reactions()
        ]
        # build a set from flatten species list of lists
        species = set([y.get_id() for x in species for y in x])
        self.assertEqual(self.pathway.get_nb_species(), len(species))

    def test_get_species(self):
        list_of_list_of_species = [
            rxn.get_species_ids() for rxn in self.reactions.values()
        ]
        list_of_species = list(
            set([spe for species in list_of_list_of_species for spe in species])
        )
    ...
    def test_get_species_from_id(self):
        self.assertEqual(self.pathway.get_specie("MNXM23"), self.species["MNXM23"])

    def test_get_species_from_id_wrong_key(self):
        self.assertEqual(self.pathway.get_specie("WRONG_KEY"), None)
    ...
    def test_get_compound(self):
        self.assertEqual(self.pathway.get_compound("MNXM23"), self.species["MNXM23"])

    def test_get_reactions(self):
        self.assertDictEqual(self.pathway.get_reactions(), self.reactions)

    def test_get_list_of_reactions(self):
        self.assertListEqual(
            self.pathway.get_list_of_reactions(), list(self.reactions.values())
        )

    def test_get_reaction(self):
        self.assertEqual(self.pathway.get_reaction("rxn_4"), self.rxn)

    def test_get_reaction_wrong_id(self):
        self.assertEqual(self.pathway.get_reaction("WRONG_ID"), None)
    ...
    def test__to_dict(self):
        ...
        self.assertDictEqual(
            ...
            {
                "reactions": {
                    rxn.get_id(): rxn._to_dict() for rxn in self.reactions.values()
                },
                "species": {
                    spe.get_id(): spe._to_dict()
                    for spe in [self.species[spe_id] for spe_id in list_of_species]
                },
    ...
    def test_not_equal_reactions(self):
        ...
        pathway.set_target_id(self.target_id)
        self.assertNotEqual(self.pathway, pathway)

    def test_del_reaction(self):
        self.assertTrue(self.pathway.del_reaction(self.rxn.get_id()))
        self.assertListEqual(
            self.pathway.get_reactions_ids(),
    ...
    def test_del_reaction_wrong_id(self):
        self.assertFalse(self.pathway.del_reaction("WRONG"))
        self.assertListEqual(
            self.pathway.get_reactions_ids(),
            [rxn.get_id() for rxn in self.reactions.values()],
        )
    ...
    def test_eq_not_equal(self):
        self.assertNotEqual(self.pathway, Pathway(id=self.id))
    ...
    def test_get_reactants(self):
        ...
        self.assertListEqual(self.pathway.get_reactants_ids(), sorted(set(species)))

    def test_get_products(self):
        species = []
        for rxn in self.reactions.values():
            species += rxn.get_products_ids()
        self.assertListEqual(self.pathway.get_products_ids(), sorted(set(species)))
