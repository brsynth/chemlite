...
class Test_Compound(TestCase):
    ...
    def test_from_dict(self):
        self.assertEqual(self.compound, Compound.from_dict(self.compound_dict))

    def test_to_string(self):
        self.assertEqual(
            self.compound.to_string(), f'Compound {self.compound_dict["id"]}'
        )

    def test__to_dict(self):
        self.assertDictEqual(self.compound._to_dict(), self.compound_dict)
    ...
    def test_eq_wrong_type(self):
        self.assertNotEqual(self.compound, 0)

    def test_get(self):
        for attr in ["smiles", "inchi", "inchikey", "name", "formula"]:
            with self.subTest(f"test_get_{attr}", attr=attr):
                self.assertEqual(
                    getattr(self.compound, f"get_{attr}")(), self.compound_dict[attr]
                )
    ...
    def test_set(self):
        ...
        for attr in ["smiles", "inchi", "inchikey", "name", "formula"]:
            with self.subTest(f"test_set_{attr}", attr=attr):
                ...
                self.assertEqual(getattr(self.compound, attr), new_str)
