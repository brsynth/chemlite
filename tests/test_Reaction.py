...
class Test_Reaction(TestCase):
    ...
    def test___init__ec_number(self):
        ec_number = "1.1.2"
        rxn = Reaction(id="rxn_test", ec_numbers=ec_number)
        self.assertListEqual(rxn.get_ec_numbers(), [ec_number])
    ...
    def test_to_string_0(self):
        ...
        self.assertEqual(self.rxn.to_string(), f"Reaction {self.rxn.get_id()}:  = ")
    ...
    def test_not_eq(self):
        ...
        self.assertNotEqual(rxn, self.rxn)

    def test_eq_wrong_type(self):
        self.assertNotEqual(self.rxn, 0)
    ...
    def test_get_reactants_ids(self):
        self.assertListEqual(self.rxn.get_reactants_ids(), list(self.reactants.keys()))

    def test_get_products_ids(self):
        self.assertListEqual(self.rxn.get_products_ids(), list(self.products.keys()))
    ...
    def test_get_smiles(self):
        self.assertEqual(self.rxn.get_smiles(), self.reactions["smiles"]["string"])

    def test_get_smiles_cmpd_wo_smiles_L(self):
        self.rxn.add_reactant(compound_id="CMPD_0000000003_wo_smiles", stoichio=1)
        self.assertEqual(self.rxn.get_smiles(), self.reactions["smiles"]["string"])

    def test_get_smiles_cmpd_wo_smiles_R(self):
        self.rxn.add_product(compound_id="CMPD_0000000003_wo_smiles", stoichio=1)
        self.assertEqual(self.rxn.get_smiles(), self.reactions["smiles"]["string"])

    def test_get_smiles_w_cmpd_not_in_cache(self):
        self.rxn.add_reactant(compound_id="CMPD_NOT_IN_CACHE", stoichio=1)
        self.assertEqual(self.rxn.get_smiles(), self.reactions["smiles"]["string"])

    def test_get_smiles_w_smile_None_L(self):
        self.rxn.add_reactant(compound_id="CMPD_0000000003_w_smiles_None", stoichio=1)
        self.assertEqual(self.rxn.get_smiles(), self.reactions["smiles"]["string"])

    def test_get_smiles_w_smile_None_R(self):
        self.rxn.add_product(compound_id="CMPD_0000000003_w_smiles_None", stoichio=1)
        self.assertEqual(self.rxn.get_smiles(), self.reactions["smiles"]["string"])

    def test_get_smiles_w_all_cmpds_wo_smiles(self):
        self.rxn.add_reactant(compound_id="CMPD_0000000003_wo_smiles", stoichio=1)
        self.rxn.add_product(compound_id="CMPD_0000000004_wo_smiles", stoichio=1)
        self.assertEqual(self.rxn.get_smiles(), self.reactions["smiles"]["string"])

    def test_get_smiles_w_floating_coeff(self):
        rxn = Reaction.from_string(id="test", rxn=self.reactions["float"]["string"])
        self.assertEqual(
            rxn.get_smiles(),
    ...
    def test_from_string(self):
        rxn = Reaction.from_string(id="test", rxn=self.reactions["float"]["string"])
        self.assertEqual(sum(rxn.get_specie("MNXM6").values()), 4)
    ...
    def test_from_string_smiles(self):
        ...
        self.assertEqual(sum(rxn.get_specie("[H]O[H]").values()), 2)
    ...
    def test_get_left(self):
        self.assertDictEqual(self.rxn.get_left(), self.reactants)

    def test_get_right(self):
        self.assertDictEqual(self.rxn.get_right(), self.products)

    def test_get_nb_species(self):
        self.assertEqual(
            self.rxn.get_nb_species(), len(self.reactants) + len(self.products)
        )

    def test_add_ec_number(self):
        ec_number = "1.2"
        self.rxn.add_ec_number(ec_number)
        self.assertListEqual(self.rxn.get_ec_numbers(), self.ec_numbers + [ec_number])
    ...
    def test_add_reactant(self):
        for spe_sto in [3, -3]:
            with self.subTest(spe_sto=spe_sto):
                rxn = deepcopy(self.rxn)
                rxn.add_reactant(
                    compound_id=self.species["MNXM337"].get_id(), stoichio=spe_sto
                )
    ...
    def test_set_product_already_exist(self):
        ...
        self.assertEqual(
            Cache.get("MNXM13").get_smiles(), self.species["MNXM13"].get_smiles()
        )
    ...
    def test_add_reactant_wo_id(self):
        ...
        self.assertDictEqual(rxn.get_reactants(), self.rxn.get_reactants())

    def test_add_reactant_with_id_none(self):
        cmpd_sto = 4
        rxn = deepcopy(self.rxn)
        rxn.add_reactant(stoichio=cmpd_sto, compound_id=None)
        self.assertListEqual(rxn.get_reactants_ids(), self.rxn.get_reactants_ids())

    def test_add_product_with_id_none(self):
        cmpd_sto = 4
        rxn = deepcopy(self.rxn)
        rxn.add_product(stoichio=cmpd_sto, compound_id=None)
        self.assertListEqual(rxn.get_products_ids(), self.rxn.get_products_ids())
    ...
    def test_rename_compound_reactant(self):
        ...
        self.assertEqual(self.rxn.get_reactants_ids(), reactants_ids)
    ...
    def test_rename_compound_product(self):
        ...
        self.assertEqual(self.rxn.get_products_ids(), products_ids)
    ...
    def test_mult_stoichio_coeff(self):
        ...
        self.assertDictEqual(
            ...
            {
                **{
                    spe_id: -mult * self.reactants[spe_id]
                    for spe_id in self.reactants.keys()
                },
                **{
                    spe_id: mult * self.products[spe_id]
                    for spe_id in self.products.keys()
                },
            },
        )

    def test_sum_stoichio(self):
        reactants_1 = {"MNXM4": 1, "MNXM337": 1, "MNXM3": 2}
        reactants_2 = {
            "MNXM4": 1,
            "MNXM337": 1,
        }
        ...
        products_1 = {"MNXM2": 1, "MNXM5": 1, "MNXM23": 1}
        products_2 = {"MNXM13": 1, "MNXM6": 3, "MNXM5": 1, "MNXM3": 1}
        products_3 = {"TARGET_0000000001": 1, "MNXM2": 1, "MNXM5": 1, "MNXM3": 1}
        rxn_1 = Reaction(
            id="rxn_1",
        ...
        self.assertDictEqual(
            Reaction.sum_stoichio([rxn_1, rxn_2, rxn_3]),
            {
                "MNXM4": -4,
                "MNXM337": -2,
                "MNXM6": 2,
